...
gset <- getGEO("GSE108000", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
```{r}
metadata=data.frame(gset$geo_accession, gset$`tissue:ch1`, gset$`gender:ch1`, gset$`age:ch1`,gset$`lesion type:ch1`, gset$`disease state:ch1`) 
head(metadata)
write.csv(metadata,'GSE108000_metadata.csv')
```
...
```{r}
#The expression data uploaded for this entry is Cy3 Cy5	microarrays 
exprList<-exprs(gset)
write.table(exprList, "GSE108000_expression_data_originalfromGEO.txt", sep="\t", quote=F)
```
...
exprRaw<-read.delim('GSE108000_expression_data_originalfromGEO.txt',header=T, sep='\t')
DT::datatable(exprRaw[1:3,])
DT::datatable(summary(exprRaw))
```

```{r}
#The original GSE108000 is in Cy3/Cy5 ratio in log scale  https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM2886523

#conversiton back to non-log scale 
exprRaw1 = 2^exprRaw 
DT::datatable(exprRaw1[1:3,])
write.table(exprRaw1, "GSE108000_expression_data_non-log.txt", sep="\t", quote=F)
```
...
```
	...
	#3.7 Visualization of GSE108000 alone
```{r}
pdf(file="Step3_plot_Visualization_GSE108000_rawdata_transformation.pdf",height=5,width=5)
par(mfrow=c(2,2))
...
## File stored at:
## /var/folders/0g/5tz57sgx7090n5vttfqvs3ym0000gn/T//Rtmpv5hBUt/GPL13497.soft
colnames(GPL_file@dataTable@table)
write.table(GPL_file@dataTable@table,"GPL13497.txt", sep='\t', quote=F)
```
