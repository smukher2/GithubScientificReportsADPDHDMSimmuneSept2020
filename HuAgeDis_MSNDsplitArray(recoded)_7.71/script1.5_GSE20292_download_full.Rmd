...
```{r}
#select samples in same series name
...
```{r}
metadata=data.frame(gset$geo_accession, gset$source_name_ch1,  gset$`age:ch1`, gset$`gender:ch1`, gset$`brain region:ch1`, gset$`disease state:ch1`)
head(metadata)
write.csv(metadata,'GSE20292_metadata.csv')
```
...
```{r}
#The expression data uploaded for this entry is Affymetrix Expression data
exprList<-exprs(gset)
write.table(exprList, "GSE20292_expression_data_originalfromGEO.txt", sep="\t", quote=F)
```
...
exprRaw<-read.delim('GSE20292_expression_data_originalfromGEO.txt',header=T, sep='\t')
DT::datatable(exprRaw[1:3,])
DT::datatable(summary(exprRaw))
```

```{r}
#The original GSE20292 is Affymetrix and (RMA)-normalized  https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM508708

#conversiton back to non-log scale
exprRaw1 = 2^exprRaw 
DT::datatable(exprRaw1[1:3,])
write.table(exprRaw1, "GSE20292_expression_data_non-log.txt", sep="\t", quote=F)
```
...
```
	...
	#3.7 Visualization of GSE20292 alone
```{r}
pdf(file="Step3_plot_Visualization_GSE20292_rawdata_transformation.pdf",height=5,width=5)
par(mfrow=c(2,2))
