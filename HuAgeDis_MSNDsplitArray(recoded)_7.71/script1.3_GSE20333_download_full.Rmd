...
gset <- getGEO("GSE20333", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
```{r}
metadata=data.frame(gset$geo_accession, gset$title, gset$source_name_ch1, gset$`gender:ch1`, gset$`age:ch1`) #title has disease and source has tissue information
head(metadata)
write.csv(metadata,'GSE20333_metadata.csv')
```
...
exprList<-exprs(gset)
write.table(exprList, "GSE20333_expression_data_originalfromGEO.txt", sep="\t", quote=F)
```
...
exprRaw<-read.delim('GSE20333_expression_data_originalfromGEO.txt',header=T, sep='\t')
DT::datatable(exprRaw[1:3,])
DT::datatable(summary(exprRaw))
```

```{r}
#The original GSE20333 is MAS 5 generated linear signals non-log scale https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM509547

#conversiton back to non-log scale exprRaw1 = 2^exprRaw is not needed
exprRaw1 = exprRaw 
DT::datatable(exprRaw1[1:3,])
write.table(exprRaw1, "GSE20333_expression_data_non-log.txt", sep="\t", quote=F)
```
...
```
	...
	#3.7 Visualization of GSE20333 alone
```{r}
pdf(file="Step3_plot_Visualization_GSE20333_rawdata_transformation.pdf",height=5,width=5)
par(mfrow=c(2,2))
...
## File stored at:
## /var/folders/0g/5tz57sgx7090n5vttfqvs3ym0000gn/T//Rtmpv5hBUt/GPL201.soft
colnames(GPL_file@dataTable@table)
write.table(GPL_file@dataTable@table,"GPL201.txt", sep='\t', quote=F)
```
