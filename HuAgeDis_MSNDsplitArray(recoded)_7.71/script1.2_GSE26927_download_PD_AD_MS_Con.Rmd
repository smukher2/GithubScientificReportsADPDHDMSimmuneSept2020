---
title: "GSE26927_download_PD_AD_MS_Con"
...
gset <- getGEO("GSE26927", GSEMatrix =TRUE, AnnotGPL=TRUE)
if (length(gset) > 1) idx <- grep("GPL6255", attr(gset, "names")) else idx <- 1
gset <- gset[[idx]] 
```

  #2.2 keep only samples of interest
```{r}
#these groups were selected on the GEO2R to keep only PDandPDCon and ADandADCon. HDandHDCon have missing age and different brain region from the rest of our study so not selected.
gsms <- paste0("000000000000000000XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
        "XXXXXXXX2222222222222222222211111111111111111111XX",
        "XXXXXXXXXXXXXXXXXX")
sml <- c()
...
```{r}
metadata=data.frame(gset$geo_accession, gset$source_name_ch1, gset$`gender:ch1`, gset$`age at death (in years):ch1`,gset$`title`, gset$`disease:ch1`) 
head(metadata)
write.csv(metadata,'GSE26927_metadata.csv')
```
...
```{r}
#The expression data uploaded for this entry is BeadChips and background corrected using Rosetta Error Models
exprList<-exprs(gset)
write.table(exprList, "GSE26927_expression_data_originalfromGEO.txt", sep="\t", quote=F)
```
...
exprRaw<-read.delim('GSE26927_expression_data_originalfromGEO.txt',header=T, sep='\t')
DT::datatable(exprRaw[1:3,])
DT::datatable(summary(exprRaw))
```

```{r}
#The original GSE26927 is from Rosetta Error Models corrected and already in non-log scale https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM1311807

#conversiton back to non-log scale, exprRaw1 = 2^exprRaw is not needed
exprRaw1 = exprRaw
DT::datatable(exprRaw1[1:3,])
write.table(exprRaw1, "GSE26927_expression_data_non-log.txt", sep="\t", quote=F)
```
...
```
	...
	#3.7 Visualization of GSE26927 alone
```{r}
pdf(file="Step3_plot_Visualization_GSE26927_rawdata_transformation.pdf",height=5,width=5)
par(mfrow=c(2,2))
...
#exprRaw1 boxplot
par(mai=c(1,0.8,1,0.8))
boxplot(exprRaw1, outline=FALSE, las=2, cex=0.25, main="exprRaw1", col="yellow")
...
## File stored at:
## /var/folders/0g/5tz57sgx7090n5vttfqvs3ym0000gn/T//Rtmpv5hBUt/GPL6255.soft
colnames(GPL_file@dataTable@table)
write.table(GPL_file@dataTable@table,"GPL6255.txt", sep='\t', quote=F)
```
...
GPL_file1<-GPL_file@dataTable@table[,c("ID","SYMBOL")]
