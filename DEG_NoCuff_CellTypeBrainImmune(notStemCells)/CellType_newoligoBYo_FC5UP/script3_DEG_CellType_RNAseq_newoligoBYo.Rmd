---
title: "script3_DEG_CellType_newoligoBYo"
...
creating_CellType_contrasts_DEG_Limma<-makeContrasts(newoligoBYo=CellTypenewoligo-CellTypeother, levels =design_DEG_Limma)
...
contrast.matrix_DEG_Limma <- cbind("newoligoBYo"=c(1,-1,rep(0,2)))
...
```{r}
#DEG_Limma for contrast "newoligoBYo"=CellTypenewoligo-CellTypeother 
result_newoligoBYo_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="newoligoBYo")
result_newoligoBYo_full_DEG_Limma$Contrast<-rep("newoligoBYo",nrow(result_newoligoBYo_full_DEG_Limma))
result_newoligoBYo_full_DEG_Limma$FC<- 2^(result_newoligoBYo_full_DEG_Limma$logFC)
result_newoligoBYo_FC5.00_DEG_Limma<- result_newoligoBYo_full_DEG_Limma[which(result_newoligoBYo_full_DEG_Limma$FC >= 5.00), ]

dim(result_newoligoBYo_full_DEG_Limma)
dim(result_newoligoBYo_FC5.00_DEG_Limma)


write.csv(result_newoligoBYo_full_DEG_Limma, "Step12_result_newoligoBYo_full_DEG_Limma.csv")
write.csv(result_newoligoBYo_FC5.00_DEG_Limma, "Step12_result_newoligoBYo_FC5.00_DEG_Limma.csv")

summary(result_newoligoBYo_full_DEG_Limma)
summary(result_newoligoBYo_FC5.00_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_newoligoBYo_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC5.00_DEG_Limma<-rbind(result_newoligoBYo_FC5.00_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"newoligoBYo"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_newoligoBYo_full_DEG_Limma
df$gene<-rownames(result_newoligoBYo_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes newoligoBYo
```{r}
#Create index to subset expression data to find the differentially expressed genes .
index_O=which(pheno$CellType=='other')
index_AS=which(pheno$CellType=='newoligo')
```

```{r}
#subset expression data to find the differentially expressed genes .
edata_newoligoBYo<-edata[,c(index_AS,index_O)]
head(edata_newoligoBYo)
```

```{r}
#Differentially expressed genes between "newoligoBYo"
t_newoligoBYo<-apply(edata_newoligoBYo,1,function(x){
  ...
  Contrast<-"newoligoBYo"
  c(meanx=meanx,meany=meany,tvalue=tvalue,FC=FC,logFC=logFC, pvalue=pvalue, Contrast=Contrast)
})
t_newoligoBYo<-t(t_newoligoBYo)
adj.P.Val<-p.adjust(t_newoligoBYo[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "newoligoBYo"
result_newoligoBYo_full_DEG_Simple<-cbind(t_newoligoBYo,adj.P.Val)
result_newoligoBYo_full_DEG_Simple<-as.data.frame(result_newoligoBYo_full_DEG_Simple)
cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
result_newoligoBYo_full_DEG_Simple[,cols] = apply(result_newoligoBYo_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_newoligoBYo_full_DEG_Simple = result_newoligoBYo_full_DEG_Simple[order(-result_newoligoBYo_full_DEG_Simple$logFC), ] #order by descending fold change

result_newoligoBYo_FC5.00_DEG_Simple<- result_newoligoBYo_full_DEG_Simple[which(result_newoligoBYo_full_DEG_Simple$FC >= 5.00), ]

dim(result_newoligoBYo_full_DEG_Simple)
dim(result_newoligoBYo_FC5.00_DEG_Simple)


write.csv(result_newoligoBYo_full_DEG_Simple, "Step15_result_newoligoBYo_full_DEG_Simple.csv")
write.csv(result_newoligoBYo_FC5.00_DEG_Simple, "Step15_result_newoligoBYo_FC5.00_DEG_Simple.csv")

summary(result_newoligoBYo_full_DEG_Simple)
summary(result_newoligoBYo_FC5.00_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_newoligoBYo_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC5.00_DEG_Simple<-rbind(result_newoligoBYo_FC5.00_DEG_Simple)#here only one table so rbind not required
...
df<-result_newoligoBYo_full_DEG_Simple
df$gene<-rownames(result_newoligoBYo_full_DEG_Simple) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
```{r}
#comparisons you want write here
creating_CellType_contrasts_DEG_edgeR<-makeContrasts(newoligoBYo=CellTypenewoligo-CellTypeother, levels =design_DEG_edgeR)
...
contrast.matrix_DEG_edgeR <- cbind("newoligoBYo"=c(1,-1,rep(0,2)))
...
lrt_newoligoBYo_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"newoligoBYo"])

```

```{r}
#DEG_edgeR for contrast "newoligoBYo"=CellTypenewoligo-CellTypeother 
result_newoligoBYo_full_DEG_edgeR<- topTags(lrt_newoligoBYo_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_newoligoBYo_full_DEG_edgeR)[names(result_newoligoBYo_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_newoligoBYo_full_DEG_edgeR$Contrast<-rep("newoligoBYo",nrow(result_newoligoBYo_full_DEG_edgeR))
result_newoligoBYo_full_DEG_edgeR$FC<- 2^(result_newoligoBYo_full_DEG_edgeR$logFC)
#drop extra genes column
result_newoligoBYo_full_DEG_edgeR <- result_newoligoBYo_full_DEG_edgeR[!names(result_newoligoBYo_full_DEG_edgeR) %in% c("genes")]
result_newoligoBYo_FC5.00_DEG_edgeR<- result_newoligoBYo_full_DEG_edgeR[which(result_newoligoBYo_full_DEG_edgeR$FC >= 5.00), ]

dim(result_newoligoBYo_full_DEG_edgeR)
dim(result_newoligoBYo_FC5.00_DEG_edgeR)


write.csv(result_newoligoBYo_full_DEG_edgeR, "Step17_result_newoligoBYo_full_DEG_edgeR.csv")
write.csv(result_newoligoBYo_FC5.00_DEG_edgeR, "Step17_result_newoligoBYo_FC5.00_DEG_edgeR.csv")

summary(result_newoligoBYo_full_DEG_edgeR)
summary(result_newoligoBYo_FC5.00_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_newoligoBYo_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC5.00_DEG_edgeR<-rbind(result_newoligoBYo_FC5.00_DEG_edgeR)#here only one table so rbind not required
...
df<-result_newoligoBYo_full_DEG_edgeR
df$gene<-rownames(result_newoligoBYo_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
