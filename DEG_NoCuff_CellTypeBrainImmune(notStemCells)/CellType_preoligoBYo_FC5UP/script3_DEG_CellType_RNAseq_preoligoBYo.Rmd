---
title: "script3_DEG_CellType_preoligoBYo"
...
creating_CellType_contrasts_DEG_Limma<-makeContrasts(preoligoBYo=CellTypepreoligo-CellTypeother, levels =design_DEG_Limma)
...
contrast.matrix_DEG_Limma <- cbind("preoligoBYo"=c(1,-1,rep(0,2)))
...
```{r}
#DEG_Limma for contrast "preoligoBYo"=CellTypepreoligo-CellTypeother 
result_preoligoBYo_full_DEG_Limma<- topTable(eb_DEG_Limma, number = nrow(edata), adjust = "BH", coef="preoligoBYo")
result_preoligoBYo_full_DEG_Limma$Contrast<-rep("preoligoBYo",nrow(result_preoligoBYo_full_DEG_Limma))
result_preoligoBYo_full_DEG_Limma$FC<- 2^(result_preoligoBYo_full_DEG_Limma$logFC)
result_preoligoBYo_FC5.00_DEG_Limma<- result_preoligoBYo_full_DEG_Limma[which(result_preoligoBYo_full_DEG_Limma$FC >= 5.00), ]

dim(result_preoligoBYo_full_DEG_Limma)
dim(result_preoligoBYo_FC5.00_DEG_Limma)


write.csv(result_preoligoBYo_full_DEG_Limma, "Step12_result_preoligoBYo_full_DEG_Limma.csv")
write.csv(result_preoligoBYo_FC5.00_DEG_Limma, "Step12_result_preoligoBYo_FC5.00_DEG_Limma.csv")

summary(result_preoligoBYo_full_DEG_Limma)
summary(result_preoligoBYo_FC5.00_DEG_Limma)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Limma<-rbind(result_preoligoBYo_full_DEG_Limma) #here only one table so rbind not required
...
result_allContrasts_FC5.00_DEG_Limma<-rbind(result_preoligoBYo_FC5.00_DEG_Limma) #here only one table so rbind not required
...
par(mfrow=c(2,2))
qqt(eb_DEG_Limma$t[,"preoligoBYo"],df_DEG_Limma=eb_DEG_Limma$df.residual+eb_DEG_Limma$df.prior)
abline(0,1)
dev.off()
```
...
df<-result_preoligoBYo_full_DEG_Limma
df$gene<-rownames(result_preoligoBYo_full_DEG_Limma) #create column for genes
...
#DEG_Simple Differentially expressed genes preoligoBYo
```{r}
#Create index to subset expression data to find the differentially expressed genes .
index_O=which(pheno$CellType=='other')
index_AS=which(pheno$CellType=='preoligo')
```

```{r}
#subset expression data to find the differentially expressed genes .
edata_preoligoBYo<-edata[,c(index_AS,index_O)]
head(edata_preoligoBYo)
```

```{r}
#Differentially expressed genes between "preoligoBYo"
t_preoligoBYo<-apply(edata_preoligoBYo,1,function(x){
  ...
  Contrast<-"preoligoBYo"
  c(meanx=meanx,meany=meany,tvalue=tvalue,FC=FC,logFC=logFC, pvalue=pvalue, Contrast=Contrast)
})
t_preoligoBYo<-t(t_preoligoBYo)
adj.P.Val<-p.adjust(t_preoligoBYo[,'pvalue'], method="BH")
```

```{r}
#DEG_Simple for contrast "preoligoBYo"
result_preoligoBYo_full_DEG_Simple<-cbind(t_preoligoBYo,adj.P.Val)
result_preoligoBYo_full_DEG_Simple<-as.data.frame(result_preoligoBYo_full_DEG_Simple)
cols = c("meanx","meany", "tvalue", "FC", "logFC", "pvalue", "adj.P.Val")  
result_preoligoBYo_full_DEG_Simple[,cols] = apply(result_preoligoBYo_full_DEG_Simple[,cols], 2, function(x) as.numeric(as.character(x)))
result_preoligoBYo_full_DEG_Simple = result_preoligoBYo_full_DEG_Simple[order(-result_preoligoBYo_full_DEG_Simple$logFC), ] #order by descending fold change

result_preoligoBYo_FC5.00_DEG_Simple<- result_preoligoBYo_full_DEG_Simple[which(result_preoligoBYo_full_DEG_Simple$FC >= 5.00), ]

dim(result_preoligoBYo_full_DEG_Simple)
dim(result_preoligoBYo_FC5.00_DEG_Simple)


write.csv(result_preoligoBYo_full_DEG_Simple, "Step15_result_preoligoBYo_full_DEG_Simple.csv")
write.csv(result_preoligoBYo_FC5.00_DEG_Simple, "Step15_result_preoligoBYo_FC5.00_DEG_Simple.csv")

summary(result_preoligoBYo_full_DEG_Simple)
summary(result_preoligoBYo_FC5.00_DEG_Simple)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_Simple<-rbind(result_preoligoBYo_full_DEG_Simple) #here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC5.00_DEG_Simple<-rbind(result_preoligoBYo_FC5.00_DEG_Simple)#here only one table so rbind not required
...
df<-result_preoligoBYo_full_DEG_Simple
df$gene<-rownames(result_preoligoBYo_full_DEG_Simple) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
...
```{r}
#comparisons you want write here
creating_CellType_contrasts_DEG_edgeR<-makeContrasts(preoligoBYo=CellTypepreoligo-CellTypeother, levels =design_DEG_edgeR)
...
contrast.matrix_DEG_edgeR <- cbind("preoligoBYo"=c(1,-1,rep(0,2)))
...
lrt_preoligoBYo_DEG_edgeR <- glmLRT(fit_DEG_edgeR, contrast=contrast.matrix_DEG_edgeR[,"preoligoBYo"])

```

```{r}
#DEG_edgeR for contrast "preoligoBYo"=CellTypepreoligo-CellTypeother 
result_preoligoBYo_full_DEG_edgeR<- topTags(lrt_preoligoBYo_DEG_edgeR, n = nrow(edata), adjust.method="BH")$table
names(result_preoligoBYo_full_DEG_edgeR)[names(result_preoligoBYo_full_DEG_edgeR) == 'FDR'] <- 'adj.P.Val'
result_preoligoBYo_full_DEG_edgeR$Contrast<-rep("preoligoBYo",nrow(result_preoligoBYo_full_DEG_edgeR))
result_preoligoBYo_full_DEG_edgeR$FC<- 2^(result_preoligoBYo_full_DEG_edgeR$logFC)
#drop extra genes column
result_preoligoBYo_full_DEG_edgeR <- result_preoligoBYo_full_DEG_edgeR[!names(result_preoligoBYo_full_DEG_edgeR) %in% c("genes")]
result_preoligoBYo_FC5.00_DEG_edgeR<- result_preoligoBYo_full_DEG_edgeR[which(result_preoligoBYo_full_DEG_edgeR$FC >= 5.00), ]

dim(result_preoligoBYo_full_DEG_edgeR)
dim(result_preoligoBYo_FC5.00_DEG_edgeR)


write.csv(result_preoligoBYo_full_DEG_edgeR, "Step17_result_preoligoBYo_full_DEG_edgeR.csv")
write.csv(result_preoligoBYo_FC5.00_DEG_edgeR, "Step17_result_preoligoBYo_FC5.00_DEG_edgeR.csv")

summary(result_preoligoBYo_full_DEG_edgeR)
summary(result_preoligoBYo_FC5.00_DEG_edgeR)
```

```{r}
#Bind all differentally expressed full genes into a single table
result_allContrasts_full_DEG_edgeR<-rbind(result_preoligoBYo_full_DEG_edgeR)#here only one table so rbind not required
...
#Bind all differentally expressed pval genes into a single table
result_allContrasts_FC5.00_DEG_edgeR<-rbind(result_preoligoBYo_FC5.00_DEG_edgeR)#here only one table so rbind not required
...
df<-result_preoligoBYo_full_DEG_edgeR
df$gene<-rownames(result_preoligoBYo_full_DEG_edgeR) #create column for genes
mutateddf <- mutate(df, sig=ifelse(df$adj.P.Val<0.05, "adj.P.Val<0.05", "Not Sig")) 
#Will have different colors depending on significance
#volcanoplot with logFC versus adj.P.Val
volc = ggplot(mutateddf, aes(logFC, -log10(adj.P.Val))) +
