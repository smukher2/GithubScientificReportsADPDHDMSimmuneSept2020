---
title: "script4_WGCNA_HuAgeDis_MSNDsplitRnaseq_v1"
...
To reuse this code for other datasets 
  a) replace "HuAgeMSNDsplitRnaseq_" for module naming to pre-fix of users choice that is meaningful for the dataset. Here Hu=Human, Age=Age, Dis=Disease
  b) in this pipeline human gene symbols are used

  
###############################################Continue with WGCNA pipeline############################################################################
####################################HuAgeMSNDsplitRnaseq#######################################
...
```{r}
#Human Metadata
metadata=read.csv("./merged_expr_metadata/Step5D_result_merge_GSE53697_to_GSE100297_metadata_SampleID.csv", header=T, sep=',')
colnames(metadata)
```
...
metadata_coded=read.csv("./merged_expr_metadata/Step8_result_merge_GSE53697_to_GSE100297_metadata_SampleID_coded_vsCONown.csv", header=T, sep=',')
...
#Order of metadata variable selection in WGCNA same as mod and then remaining variables
#keep only variable we need
metadata_1_coded=metadata_coded[,c("StudyGSE53697","StudyGSE64810","StudyGSE68719","StudyGSE67333","GenderM","DiseaseCON","DiseaseAD","DiseaseHD","DiseasePD","DiseaseMS","Age","Tissueprefrontal_cortex")]
...
diag(adjacencyA1)=0
#Turn adjacency into topology overlap matrix y uncommenting line below
...
mColorhL=cbind(mColorhL, paste("HuAgeMSNDsplitRnaseq_", str_pad(tree$labels, 2, pad="0"), sep="" ))
}
...
write.csv(datExprA1g2,"Step20_result_ForPres&Hyper_HuAgeMSNDsplitRnaseq_datExprA1g2.csv")
#Export the module and gene names for use Preservation and hypergeometric test 
write.csv(kMEtable1[,c("PSID","Gene","Module")],"Step20_result_ForPres&Hyper_HuAgeMSNDsplitRnaseq_GeneModule.csv")
write.csv(kMEtable1L[,c("PSID","Gene","Module")],"Step20_result_ForPres&Hyper_HuAgeMSNDsplitRnaseq_GeneModuleL.csv")
```

###Step21A: Enrichment of cell types in modules###

```{r}
celltype=read.csv("./InputMetadata&Lists/CellType_ImmuneCells_StemCells_Full_Hu.csv",sep=',')
cellTypeGeneCount=as.data.frame(table(celltype$Category))
cellTypeGeneCount
write.csv(cellTypeGeneCount,"CellType_ImmuneCells_StemCells_Full_Hu_GeneCount.csv")
```

```{r}
#This is the actual calulation of hypergeometric enrichment
enrichmentsCellTypeFullHuman = userListEnrichment(rownames(datExprA1g2), modulesA1,"./InputMetadata&Lists/CellType_ImmuneCells_StemCells_Full_Hu.csv", "", "enrichmentsCellTypeFullHuman_Sigificantcolors.csv")
...
#Same as above for labeled modules
```{r}
enrichmentsCellTypeFullHumanL = userListEnrichment(rownames(datExprA1g2), modulesA1L,"./InputMetadata&Lists/CellType_ImmuneCells_StemCells_Full_Hu.csv", "", "enrichmentsCellTypeFullHuman_Sigificantlabels.csv")
...
##############################DEGup enrichment in modules#################################
###Step21B: Enrichment test for comparison with Cuffdiff2 defined DEGup signature genes ###

```{r}
#This is the actual calulation of hypergeometric enrichment
enrichmentsDEGup = userListEnrichment(rownames(datExprA1g2), modulesA1,"./InputMetadata&Lists/Disease_Microarray_DEGup_FC1.25_Signature.csv", "", "enrichmentsDEGup_Sigificantcolors.csv")

#List of genes that overlap between modules and DEGup
enrichmentsDEGup_OvGenes=enrichmentsDEGup[[2]]
dir.create("./enrichmentsDEGup_OvGenes")
for (i in 1:length(enrichmentsDEGup_OvGenes)) {
  write.csv(enrichmentsDEGup_OvGenes[i], file=paste0("enrichmentsDEGup_OvGenes/", names(enrichmentsDEGup_OvGenes)[i], ".txt"))
}

#List of genes that overlap between modules and DEGup
enrichmentsDEGup_NumOvGenes_pvalue=enrichmentsDEGup[[1]]
write.csv(enrichmentsDEGup_NumOvGenes_pvalue,"enrichmentsDEGup_NumOvGenes_pvalue_colors.csv")
```

```{r}
geneListsModuleSummary=read.csv('Step18_result_geneListsModuleSummarycolors.csv', sep=',')
enrichmentsSummaryDEGup=read.csv('enrichmentsDEGup_NumOvGenes_pvalue_colors.csv', sep=',')
```
...
colnames(geneListsModuleSummaryMod)=c("InputCategories", "Freq")
geneListsModuleSummaryMod
enrichmentsSummaryDEGupmergeMod=merge(enrichmentsSummaryDEGup,geneListsModuleSummaryMod, by="InputCategories")
write.csv(enrichmentsSummaryDEGupmergeMod,"enrichmentsSummaryDEGupmergeMod_colors.csv")
```

#Same as above for labeled modules
```{r}
enrichmentsDEGupL = userListEnrichment(rownames(datExprA1g2), modulesA1L,"./InputMetadata&Lists/Disease_Microarray_DEGup_FC1.25_Signature.csv", "", "enrichmentsDEGup_Sigificantlabels.csv")

#List of genes that overlap between modules and DEGup
enrichmentsDEGup_OvGenesL=enrichmentsDEGupL[[2]]
for (i in 1:length(enrichmentsDEGup_OvGenesL)) {
  write.csv(enrichmentsDEGup_OvGenesL[i], file=paste0("enrichmentsDEGup_OvGenes/", names(enrichmentsDEGup_OvGenesL)[i], ".txt"))
}

#List of genes that overlap between modules and DEGup
enrichmentsDEGup_NumOvGenes_pvalueL=enrichmentsDEGupL[[1]]
write.csv(enrichmentsDEGup_NumOvGenes_pvalueL,"enrichmentsDEGup_NumOvGenes_pvalue_labels.csv")
```

```{r}
geneListsModuleSummaryL=read.csv('Step18_result_geneListsModuleSummarylabels.csv', sep=',')
enrichmentsSummaryDEGupL=read.csv('enrichmentsDEGup_NumOvGenes_pvalue_labels.csv', sep=',')
```
...
colnames(geneListsModuleSummaryModL)=c("InputCategories", "Freq")
enrichmentsSummaryDEGupmergeModL=merge(enrichmentsSummaryDEGupL,geneListsModuleSummaryModL, by="InputCategories")
write.csv(enrichmentsSummaryDEGupmergeModL,"enrichmentsSummaryDEGupmergeModL_labels.csv")
```

##############################DEGupCON enrichment in modules#################################
###Step21C: Enrichment test for comparison with Cuffdiff2 defined DEGupCON signature genes ###

```{r}
#This is the actual calulation of hypergeometric enrichment
enrichmentsDEGupCON = userListEnrichment(rownames(datExprA1g2), modulesA1,"./InputMetadata&Lists/Disease_Microarray_DEGupCON_FC1.25_Signature.csv", "", "enrichmentsDEGupCON_Sigificantcolors.csv")

#List of genes that overlap between modules and DEGupCON
enrichmentsDEGupCON_OvGenes=enrichmentsDEGupCON[[2]]
dir.create("./enrichmentsDEGupCON_OvGenes")
for (i in 1:length(enrichmentsDEGupCON_OvGenes)) {
  write.csv(enrichmentsDEGupCON_OvGenes[i], file=paste0("enrichmentsDEGupCON_OvGenes/", names(enrichmentsDEGupCON_OvGenes)[i], ".txt"))
}

#List of genes that overlap between modules and DEGupCON
enrichmentsDEGupCON_NumOvGenes_pvalue=enrichmentsDEGupCON[[1]]
write.csv(enrichmentsDEGupCON_NumOvGenes_pvalue,"enrichmentsDEGupCON_NumOvGenes_pvalue_colors.csv")
```

```{r}
geneListsModuleSummary=read.csv('Step18_result_geneListsModuleSummarycolors.csv', sep=',')
enrichmentsSummaryDEGupCON=read.csv('enrichmentsDEGupCON_NumOvGenes_pvalue_colors.csv', sep=',')
```
...
colnames(geneListsModuleSummaryMod)=c("InputCategories", "Freq")
geneListsModuleSummaryMod
enrichmentsSummaryDEGupCONmergeMod=merge(enrichmentsSummaryDEGupCON,geneListsModuleSummaryMod, by="InputCategories")
write.csv(enrichmentsSummaryDEGupCONmergeMod,"enrichmentsSummaryDEGupCONmergeMod_colors.csv")
```

#Same as above for labeled modules
```{r}
enrichmentsDEGupCONL = userListEnrichment(rownames(datExprA1g2), modulesA1L,"./InputMetadata&Lists/Disease_Microarray_DEGupCON_FC1.25_Signature.csv", "", "enrichmentsDEGupCON_Sigificantlabels.csv")

#List of genes that overlap between modules and DEGupCON
enrichmentsDEGupCON_OvGenesL=enrichmentsDEGupCONL[[2]]
for (i in 1:length(enrichmentsDEGupCON_OvGenesL)) {
  write.csv(enrichmentsDEGupCON_OvGenesL[i], file=paste0("enrichmentsDEGupCON_OvGenes/", names(enrichmentsDEGupCON_OvGenesL)[i], ".txt"))
}

#List of genes that overlap between modules and DEGupCON
enrichmentsDEGupCON_NumOvGenes_pvalueL=enrichmentsDEGupCONL[[1]]
write.csv(enrichmentsDEGupCON_NumOvGenes_pvalueL,"enrichmentsDEGupCON_NumOvGenes_pvalue_labels.csv")
```

```{r}
geneListsModuleSummaryL=read.csv('Step18_result_geneListsModuleSummarylabels.csv', sep=',')
enrichmentsSummaryDEGupCONL=read.csv('enrichmentsDEGupCON_NumOvGenes_pvalue_labels.csv', sep=',')
```
...
colnames(geneListsModuleSummaryModL)=c("InputCategories", "Freq")
enrichmentsSummaryDEGupCONmergeModL=merge(enrichmentsSummaryDEGupCONL,geneListsModuleSummaryModL, by="InputCategories")
write.csv(enrichmentsSummaryDEGupCONmergeModL,"enrichmentsSummaryDEGupCONmergeModL_labels.csv")
```
...
# Basic barplot
folder=getwd()
pdf(paste0("Step21_plot_WGCNA_NumberOfModules_plot_", "blue",".pdf"), height=10, width=5)
p1<-ggplot(data=geneListsModuleSummaryModL, aes(x=InputCategories, y=Freq, fill="blue"))+
...
#here all modules are significant for atleast one of the diseases except _01
sig_mod_names<-c("HuAgeMSNDsplitRnaseq_02","HuAgeMSNDsplitRnaseq_03","HuAgeMSNDsplitRnaseq_04","HuAgeMSNDsplitRnaseq_05","HuAgeMSNDsplitRnaseq_06","HuAgeMSNDsplitRnaseq_07","HuAgeMSNDsplitRnaseq_08","HuAgeMSNDsplitRnaseq_09","HuAgeMSNDsplitRnaseq_10","HuAgeMSNDsplitRnaseq_11","HuAgeMSNDsplitRnaseq_12","HuAgeMSNDsplitRnaseq_13","HuAgeMSNDsplitRnaseq_14","HuAgeMSNDsplitRnaseq_15","HuAgeMSNDsplitRnaseq_16","HuAgeMSNDsplitRnaseq_17","HuAgeMSNDsplitRnaseq_18","HuAgeMSNDsplitRnaseq_19","HuAgeMSNDsplitRnaseq_20","HuAgeMSNDsplitRnaseq_21","HuAgeMSNDsplitRnaseq_22","HuAgeMSNDsplitRnaseq_23","HuAgeMSNDsplitRnaseq_24","HuAgeMSNDsplitRnaseq_25","HuAgeMSNDsplitRnaseq_26","HuAgeMSNDsplitRnaseq_27","HuAgeMSNDsplitRnaseq_28")

geneListsModuleSummaryModLnew=geneListsModuleSummaryModL[geneListsModuleSummaryModL$InputCategories %in% sig_mod_names,]

folder=getwd()
pdf(paste0("Step21_plot_WGCNA_NumberOfModules_plot_significant_", "blue",".pdf"), height=5, width=10)
p1<-ggplot(data=geneListsModuleSummaryModLnew, aes(x=InputCategories, y=Freq, fill="blue"))+
  geom_bar(position="stack", stat="identity", fill = "blue")+
  labs(title=paste0(folder,"_","WGCNA_NumberOfModules_plot"))+
  labs(x="Module names")+
...
#Step 22: Organization and saving session (software version) information 
```{r}
sessionInfo()
toLatex(sessionInfo())
```
...
dir.create("enrichmentsDEGup_OvGenes")
file.move(list.files(pattern = "enrichmentsDEGup_Num*"), "enrichmentsDEGup_OvGenes")
file.move(list.files(pattern = "enrichmentsDEGup_Sig*"), "enrichmentsDEGup_OvGenes")
file.move(list.files(pattern = "enrichmentsSummaryDEGup*"), "enrichmentsDEGup_OvGenes")

dir.create("enrichmentsDEGupCON_OvGenes")
file.move(list.files(pattern = "enrichmentsDEGupCON_Num*"), "enrichmentsDEGupCON_OvGenes")
file.move(list.files(pattern = "enrichmentsDEGupCON_Sig*"), "enrichmentsDEGupCON_OvGenes")
file.move(list.files(pattern = "enrichmentsSummaryDEGupCON*"), "enrichmentsDEGupCON_OvGenes")
