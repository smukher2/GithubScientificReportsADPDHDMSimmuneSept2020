---
title: "Pres&Hyper_HuAgeMSNDsplitVsHuAgeMSNDsplitRnaseq(color)"
...
dat1colors:'Step20_result_ForPres&Hyper_HuAgeMSNDsplit_GeneModule.csv' and 'Step20_result_ForPres&Hyper_HuAgeMSNDsplit_datExprA1g2.csv' from 'HuAgeDis_MSNDsplitArray(recoded)_7.71'
...
dat2labels:'Step20_result_ForPres&Hyper_HuAgeMSNDsplitRnaseq_GeneModuleL.csv' and 'Step20_result_ForPres&Hyper_HuAgeMSNDsplitRnaseq_datExprA1g2.csv' from 'HuAgeDis_MSNDsplitRnaseq(recoded)_v7.71PDstudyotherCONs_minMod100'
  ...
  #2) data 'dat1colors' is the data from which we input the colors or module definition of the preservation analysis
...
###Step2: Preparation of data for preservation analysis dat1colors###
#Import dat1colors for analysis
```{r}
#Obtain this listMod of choice from the WGCNA pipeline to which the dataset needs to be compared with
dat1colors=read.csv('./InputPreservation/Step20_result_ForPres&Hyper_HuAgeMSNDsplit_GeneModule.csv', sep=',')
DT::datatable(dat1colors[1:3,])
```

```{r}
# This module assignment colors. Check column name and label appropriately 
dat1colors = dat1colors$Module
head(dat1colors)
```
...
###Step 4: Preservation analysis with dat1colors vs dat2labels HuAgeMSNDsplitRnaseq###
#Calculation of module preservation using first dataset or previous analysis module colors
...
```{r}
setLabels1 = c("HuAgeMSNDsplit_colors", "HuAgeMSNDsplitRnaseq_labels");
multiExpr1 = list(A = list(data = dat1Expr), B = list(data = dat2Expr));
multiColor1 = list(A = dat1colors)
```

```{r}
system.time( {
mp1 = modulePreservation(multiExpr1, multiColor1,
referenceNetworks = 1,
nPermutations = 100,
...
```{r}
#Analysis and display of module preservation results using previous module colors
ref = 1
test = 2
...
modColors1 = rownames(mp1$preservation$observed[[ref]][[test]])
moduleSizes1 = mp1$preservation$Z[[ref]][[test]][, 1];
# leave grey and gold modules out
plotMods1 = !(modColors1 %in% c("grey", "gold"));
# Text labels for points
text1 = modColors1[plotMods1];
...
# Start the plot
sizeGrWindow(10, 5);
pdf(file="Step4_plot_HuAgeMSNDsplit_colors_vs_HuAgeMSNDsplitRnaseq_labels_preservation.pdf", width=30, height=15)
par(mfrow = c(1,2))
...
plot(moduleSizes1[plotMods1], plotData1[plotMods1, p], col = 1, bg = modColors1[plotMods1], pch = 21,
main = mains[p],
cex = 2,
...
write.csv(mp_Zsummary, "Step4_result_HuAgeMSNDsplit_colors_vs_HuAgeMSNDsplitRnaseq_labels_preservation_Zsummary.csv")
```
...
write.csv(mp_logp_Bonfsummary[,-c(1)], "Step4_result_HuAgeMSNDsplit_colors_vs_HuAgeMSNDsplitRnaseq_labels_preservation_logP_Bonfsummary.csv")
```

###Step 5: Enrichment test for comparison with dat1colors vs dat2labels HuAgeMSNDsplitRnaseq ###
```{r}
...
modulesA1=dat1colors
enrichmentsdat2labels = userListEnrichment(colnames(dat1Expr), modulesA1,"Step5_result_ForEnrich_dat2HuAgeMSNDsplitRnaseqlabels_Hu.csv", "", "enrichmentsdat2HuAgeMSNDsplitRnaseqlabels_colors.csv")
```
...
write.csv(enrichmentsdat2labels_NumOvGenes_pvalue,"enrichmentsdat2HuAgeMSNDsplitRnaseq_NumOvGenes_pvalue_colors.csv")
```
...
file.move(list.files(pattern = "enrichmentsdat2HuAgeMSNDsplitRnaseqlabels_colors.csv"), "enrichmentsdat2HuAgeMSNDsplitRnaseqlabels_OvGenes")
file.move(list.files(pattern = "enrichmentsdat2HuAgeMSNDsplitRnaseq_NumOvGenes_pvalue_colors.csv"), "enrichmentsdat2HuAgeMSNDsplitRnaseqlabels_OvGenes")
...
file.move(list.files(pattern = "HuAgeMSNDsplit_colors_vs_HuAgeMSNDsplitRnaseq_labels*"), "PreservationResults")
```
